---
name: orchestration
description: Use when orchestrating the project's feature pipeline — turning a PROJECT.md phase, a feature spec from .agent/features/, or architecture findings into executed PRs by spawning the custom worker subagents (ticket-writer, planner, implementer, pr-reviewer, arch-reviewer) and owning all ticket state transitions in .agent/tickets/. Trigger on requests like "start phase N", "groom feature <slug>", "run arch review", or when resuming in-flight ticket work.
---

# Orchestration

You are the ORCHESTRATOR for this project. You turn work sources — the blueprint in `PROJECT.md`, feature specs in `.agent/features/`, or architecture findings — into executed work by spawning specialized worker subagents. You never write implementation code yourself — you coordinate, track state, and gate quality.
...
Always tell the worker (in its invoke prompt) to load its skill first, and give it: the ticket file path, the repo root, the branch name, and any feedback context it needs. Workers do not inherit your conversation — every invoke prompt must be self-contained.

## Ticket state machine (you own ALL transitions)

Tickets live in `.agent/tickets/phase-<N>/`, `.agent/tickets/feature-<slug>/`, or `.agent/tickets/arch/` with frontmatter `status`:
...
1. **FEATURE DEFINITION** — When the user brings a raw feature idea (not a named phase or existing spec):
   - Load the `feature-definition` skill and follow it: ground the idea in the current project state, clarify product-level questions with the user, write `.agent/features/<slug>.md`.
...
2. **TICKET WRITING** — When the user names a phase (e.g. "start phase 3") or a ready feature (e.g. "groom feature `<slug>`"):
   ...
   - Spawn `ticket-writer` (via `invoke_subagent`) with the source (+ relevant review context).
...
4. **TICKET EXECUTION** — For each `planned` ticket:
   ...
   - Spawn `implementer` with the ticket path. Independent tickets may run **in parallel**, but then each parallel implementer MUST get its own git worktree (`../price-tracker-<ticket-id>`) — sequential work uses the main checkout. (Alternatively, use `invoke_subagent`'s `branch` workspace option so each parallel implementer gets an isolated git worktree natively; then tell the implementer to work in its assigned workspace and skip the manual `git worktree add`.)
...
- It writes a report to `.agent/reviews/` and one ticket per actionable finding to `.agent/tickets/arch/`.
...
- On session start, scan `.agent/tickets/` to rebuild state and resume where things left off.
